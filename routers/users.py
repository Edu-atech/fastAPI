...
@router.get("/user/")
async def user_by_param(user_id: int, db: Session = Depends(get_db)):
    user_model = db.query(models.Users).filter(models.Users.id == user_id).first()
    if user_model is not None:
        return user_model
    return "Invalid user_id"
...
async def user_password_change(
    ...
    if user_model is not None:
        if user_verificaton.username == user_model.username and verify_password(
            user_verificaton.password, user_model.hashed_password
        ):
            user_model.hashed_password = get_password_hash(
                user_verificaton.new_password
            )
            db.add(user_model)
            db.commit()
            return "Successful"
